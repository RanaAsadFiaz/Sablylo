...
export class NavigationComponent implements OnInit, OnDestroy {
  ...
  navModel: NavigationModel = {} as NavigationModel;
  isOwner = false;
      ...
      this.navService.data.subscribe((res) => {
        if (res && res !== 1) {
          if (res.entities.length === 0) {
            ...
            const index = this.helperService.findIndex(this.navModel.entityUserData, function(entity) {
  ...
  getCurrentUser() {
    ...
    //     const userData = this.compiler.constructProfileData(this.profileModel.dataRecieved.data.user);
      ...
      this.subs.add(
        this.navService.changeActiveEntity(this.navModel.selectedEntity.entityInfo.id).subscribe((res) => {
          if (res && res.responseDetails.code === 100) {
            const permissionData = JSON.stringify(this.navModel.selectedEntity.permissions);
  ...
  //  */
  // showModel(isProfile) {
  //   const modal = (isProfile) ? GeneralComponent : SecurityComponent;
  ...
  getRoleFromStorage() {
    const currentRole = this.helperService.decrypt(localStorage.getItem
